...
foreach (config('tenancy.central_domains') as $domain) {
    Route::domain($domain)->middleware('web')->group(function () {
        ...
        Route::get('/pricing', [TenantRegistrationController::class, 'pricing'])->name('shop.pricing');
        Route::get('/register-shop', [TenantRegistrationController::class, 'create'])->name('shop.register');
        Route::post('/register-shop', [TenantRegistrationController::class, 'store']);
        Route::get('/shop-pending', [TenantRegistrationController::class, 'pending'])->name('shop.pending');
        ...
        Route::prefix('admin')->group(function () {
            ...
            Route::middleware('admin.auth')->group(function () {
                ...
                Route::patch('/tenants/{tenant}/features', [TenantController::class, 'updateFeatures'])->name('admin.tenants.update-features');
                Route::patch('/tenants/{tenant}/plan', [TenantController::class, 'updatePlan'])->name('admin.tenants.update-plan');
                Route::patch('/tenants/{tenant}/mark-paid', [TenantController::class, 'markPaid'])->name('admin.tenants.mark-paid');
                Route::patch('/tenants/{tenant}/mark-unpaid', [TenantController::class, 'markUnpaid'])->name('admin.tenants.mark-unpaid');
                ...
                // Profile
                Route::get('/settings/profile', [AdminProfileController::class, 'edit'])->name('admin.settings.profile');
                Route::patch('/settings/profile', [AdminProfileController::class, 'update'])->name('admin.settings.profile.update');
                Route::put('/settings/password', [AdminProfileController::class, 'updatePassword'])->name('admin.settings.password');

                // Theme & Logo
                Route::get('/settings/theme', [AdminSettingsController::class, 'index'])->name('admin.settings.theme');
                Route::patch('/settings/theme', [AdminSettingsController::class, 'updateTheme'])->name('admin.settings.theme.update');
                Route::post('/settings/logo', [AdminSettingsController::class, 'updateLogo'])->name('admin.settings.logo');
                Route::delete('/settings/logo', [AdminSettingsController::class, 'removeLogo'])->name('admin.settings.logo.remove');
            });
        });
    });
}
