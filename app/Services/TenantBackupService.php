...
class TenantBackupService
{
    ...
    private function backupMySqlDatabase(array $connection, string $dbName, string $filepath): string
    {
        ...
        if ($result['exit_code'] !== 0) {
            ...
            if ($this->shouldUsePhpMySqlDumpFallback($result)) {
                Log::warning('mysqldump failed with auth plugin mismatch; using PHP SQL backup fallback.', [
                    'database' => $dbName,
                    'stderr' => $result['stderr'],
                ]);

                return $this->backupMySqlDatabaseWithPhp($filepath);
            }
    ...
    /**
     * Detect whether mysqldump failed due to client auth plugin mismatch.
     */
    private function shouldUsePhpMySqlDumpFallback(array $result): bool
    {
        $output = strtolower(trim(($result['stderr'] ?? '') . ' ' . ($result['stdout'] ?? '')));

        return str_contains($output, 'caching_sha2_password')
            || str_contains($output, 'plugin')
            && str_contains($output, 'could not be loaded');
    }

    /**
     * Backup MySQL database by generating SQL from the active tenant connection.
     */
    private function backupMySqlDatabaseWithPhp(string $filepath): string
    {
        $connection = DB::connection();
        $pdo = $connection->getPdo();

        $sql = [];
        $sql[] = '-- SQL backup generated by TenantBackupService fallback';
        ...
        foreach ($tables as $tableRow) {
            ...
            $wrappedTable = $this->wrapSqlIdentifier($tableName);

            $createTableRows = $connection->select("SHOW CREATE TABLE {$wrappedTable}");
            $createTableSql = (string) array_values((array) $createTableRows[0])[1];

            $sql[] = "DROP TABLE IF EXISTS {$wrappedTable};";
            $sql[] = $createTableSql . ';';
            ...
            if ($rows->isNotEmpty()) {
                ...
                $wrappedColumns = implode(', ', array_map([$this, 'wrapSqlIdentifier'], $columns));

                foreach ($rows as $row) {
                    $values = [];

                    foreach ((array) $row as $value) {
                        $values[] = $this->toSqlValue($pdo, $value);
                    }

                    $sql[] = "INSERT INTO {$wrappedTable} ({$wrappedColumns}) VALUES (" . implode(', ', $values) . ');';
                }
            }
        ...
        File::put($filepath, implode(PHP_EOL, $sql));

        return $filepath;
    }

    /**
     * Wrap SQL identifiers with escaped backticks.
     */
    private function wrapSqlIdentifier(string $identifier): string
    {
    ...
    private function restoreMySqlDatabase(array $connection, string $dbName, string $sqlFile): void
    {
        ...
        if ($result['exit_code'] !== 0) {
            if ($this->shouldUsePhpMySqlDumpFallback($result)) {
                Log::warning('mysql client restore failed with auth plugin mismatch; using PHP SQL restore fallback.', [
                    'database' => $dbName,
                    'stderr' => $result['stderr'],
                ]);

                $this->restoreMySqlDatabaseWithPhp($sqlFile);
    ...
    /**
     * Restore MySQL database using direct SQL execution from file.
     */
    private function restoreMySqlDatabaseWithPhp(string $sqlFile): void
    {
