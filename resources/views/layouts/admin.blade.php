@php
    ...
    $logoUrl = $admin?->logo_path && Storage::disk('public')->exists($admin->logo_path)
        ? Storage::disk('public')->url($admin->logo_path)
        : null;
    ...
    $desktopSidebarWidthClass = $isCompactSidebar ? 'lg:w-20' : 'lg:w-64';
    $contentOffsetClass = match (true) {
        $isSidebarRight && $isCompactSidebar => 'lg:pr-28',
        $isSidebarRight && $isFloatingSidebar => 'lg:pr-80',
        $isSidebarRight => 'lg:pr-64',
        ! $isSidebarRight && $isCompactSidebar => 'lg:pl-28',
        ! $isSidebarRight && $isFloatingSidebar => 'lg:pl-80',
        default => 'lg:pl-64',
    };
            ...
            <aside :class="sidebarOpen ? 'translate-x-0' : '{{ $mobileClosedSidebarClass }}'" class="fixed inset-y-0 {{ $mobileSidebarPositionClass }} {{ $desktopSidebarPositionClass }} {{ $floatingDesktopPositionClass }} z-50 w-64 {{ $desktopSidebarWidthClass }} flex flex-col {{ $sidebarSurfaceClasses }} transform transition-transform duration-300 ease-in-out lg:translate-x-0">
                <div class="flex min-h-[5.5rem] items-center justify-between border-b border-gray-200 px-6 pt-5 pb-6 dark:border-slate-800">
                    <a href="{{ route('admin.dashboard') }}" class="flex min-w-0 items-center gap-2">
                        @if ($logoVisible)
                            <img src="{{ $logoUrl }}" alt="Admin Logo" class="h-8 w-8 flex-shrink-0 rounded-xl object-contain">
                        @endif
                ...
                <nav class="flex-1 space-y-1 overflow-y-auto px-4 py-4">
                    ...
                    <a href="{{ route('admin.registrations.index') }}" title="Shop Registrations" class="flex items-center justify-between px-3 py-2 text-sm font-medium rounded-md {{ request()->routeIs('admin.registrations.*') ? $activeNavClass : $inactiveNavClass }}">
                        <div class="flex items-center {{ $navAlignmentClass }}">
                            <svg class="{{ $iconSpacingClass }}" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>
                            <span class="{{ $navLabelVisibilityClass }}">Shop Registrations</span>
                        </div>
                        @if ($pendingCount > 0)
                            <span class="ml-2 inline-flex items-center justify-center rounded-full bg-yellow-400 px-2 py-0.5 text-xs font-medium text-white {{ $navLabelVisibilityClass }}">
                                {{ $pendingCount }}
                            </span>
                        @endif
                    </a>
                    ...
                    <a href="{{ route('admin.subscription-plans.index') }}" title="Subscription Plans" class="flex items-center px-3 py-2 text-sm font-medium rounded-md {{ $navAlignmentClass }} {{ request()->routeIs('admin.subscription-plans.*') ? $activeNavClass : $inactiveNavClass }}">
                        ...
                        <span class="{{ $navLabelVisibilityClass }}">Subscription Plans</span>
                    </a>
                    ...
                    <a href="{{ route('admin.tenants.index') }}" title="Shops" class="flex items-center px-3 py-2 text-sm font-medium rounded-md {{ $navAlignmentClass }} {{ request()->routeIs('admin.tenants.*') ? $activeNavClass : $inactiveNavClass }}">
                        ...
                        <span class="{{ $navLabelVisibilityClass }}">Shops</span>
                    </a>
