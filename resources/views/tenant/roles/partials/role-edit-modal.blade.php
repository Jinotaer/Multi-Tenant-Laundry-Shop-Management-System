@php
    $isEditingThisRole = old('form_context') === 'role-edit' && (int) old('editing_role_id') === $role->id;
    $selectedPermissionKeys = collect($isEditingThisRole ? old('permissions', []) : $role->permissions->pluck('key')->all());
@endphp

<x-modal name="role-edit-{{ $role->id }}-modal" :show="$errors->getBag('role')->isNotEmpty() && $isEditingThisRole" maxWidth="2xl" focusable>
    <form
        method="POST"
        action="{{ route('tenant.roles.update', $role) }}"
        class="p-6"
        x-data="{
            selectedPermissions: @js($selectedPermissionKeys->values()->all()),
    ...
    >
        @csrf
        @method('PATCH')
        <input type="hidden" name="form_context" value="role-edit">
        ...
        <div class="flex items-start justify-between gap-4">
            <div>
                ...
                <h3 class="mt-2 text-lg font-semibold text-gray-900">Edit Role</h3>
                <p class="mt-1 text-sm text-gray-500">Update the details and permissions for {{ $role->name }}.</p>
            </div>
        ...
        <div class="mt-6 space-y-5">
            <div>
                ...
                <input
                    type="text"
                    name="name"
                    value="{{ $isEditingThisRole ? old('name', $role->name) : $role->name }}"
                    required
            ...
            <div>
                ...
                >{{ $isEditingThisRole ? old('description', $role->description) : $role->description }}</textarea>
                @if ($isEditingThisRole)
            ...
            <div>
                ...
                <div class="max-h-96 space-y-4 overflow-y-auto rounded-2xl border border-gray-200 bg-gray-50 p-4">
                    @foreach ($rolePermissionsByModule as $module => $permissions)
                        <div>
                                ...
                                @foreach ($permissions as $permission)
                                    <label class="ml-7 flex items-center gap-2 text-sm text-gray-700">
                                        <input
                                            ...
                                            @checked($selectedPermissionKeys->contains($permission->key))
