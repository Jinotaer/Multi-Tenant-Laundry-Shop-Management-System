                ...
                <form method="POST" action="{{ route('tenant.orders.update', $order) }}" class="space-y-6"
                    x-data="{
                        ...
                        get serviceTotal() {
                            ...
                            if (this.selectedService.price_type === 'per_kilo') return parseFloat(this.selectedService.price) * parseFloat(this.weight || 0);
                        ...
                        <div>
                            ...
                            <select name="service_id" x-model="serviceId"
                                ...
                                @foreach ($services as $service)
                                    <option value="{{ $service->id }}">{{ $service->name }} (₱{{ number_format($service->price, 2) }}/{{ $service->price_type }})</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <template x-if="selectedService">
                        <p class="text-sm text-gray-500">Service cost: <span class="font-medium" x-text="'₱' + serviceTotal.toFixed(2)"></span></p>
                    </template>
                            ...
                            <template x-for="(item, index) in items" :key="index">
                                <div class="grid grid-cols-12 gap-2 items-center">
                                    ...
                                    <div class="col-span-3">
                                        <input type="number" :name="`items[${index}][price]`" x-model="item.price" min="0" step="0.01" placeholder="Price (₱)"
                                            class="block w-full rounded-md border-gray-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    </div>
                    ...
                    <div class="flex items-center justify-between rounded-lg bg-gray-50 px-4 py-3">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <input type="number" name="total_amount" :value="total.toFixed(2)" x-model="total"
