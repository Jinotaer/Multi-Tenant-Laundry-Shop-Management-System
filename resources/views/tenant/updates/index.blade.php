<x-tenant-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                        ...
                        <div class="flex items-start justify-between gap-4">
                            <div class="flex-1">
                                <div class="flex items-center gap-2 mb-2">
                                    <h4 class="font-bold text-gray-900 dark:text-slate-100">{{ $update->release->version_tag }}</h4>
                                    @if($update->release->is_required)
                                    ...
                                    @endif
                                    @if($update->release->is_prerelease)
                                ...
                                <p class="text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">{{ $update->release->name }}</p>
                                <p class="text-xs text-gray-500 dark:text-slate-400 mb-3">Published {{ $update->release->published_at->diffForHumans() }}</p>
                                <div class="text-sm text-gray-700 dark:text-slate-300 prose prose-sm dark:prose-invert max-w-none">
                                    {!! Str::markdown($update->release->body ?? 'No release notes available.') !!}
                                </div>
                            </div>
                            <div class="flex-shrink-0">
                                <form action="{{ route('tenant.updates.apply', $update->release->id) }}" method="POST" onsubmit="return confirm('A backup will be created before updating. Continue?');">
                                    @csrf
            ...
            <div class="p-4 sm:p-8 bg-white dark:bg-slate-900 shadow sm:rounded-lg">
                <h3 class="text-lg font-medium text-gray-900 dark:text-slate-100 mb-4">Version History & Rollbacks</h3>
                <p class="text-sm text-gray-600 dark:text-slate-400 mb-4">View your update history and rollback to previous versions if needed.</p>
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-slate-400">
                            ...
                            <tbody>
                                ...
                                <tr class="bg-white dark:bg-slate-900 border-b dark:border-slate-800">
                                    ...
                                    <td class="px-6 py-4">
                                        @if(!$history->is_current)
                                        <form action="{{ route('tenant.updates.rollback', $history->release->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to rollback to this version? A backup will be created first.');">
                                            @csrf
                                            <button type="submit" class="font-medium text-red-600 dark:text-red-400 hover:underline">Rollback</button>
                                        </form>
                                        @else
                                        <span class="text-gray-400 dark:text-slate-500">Current</span>
                                        @endif
                                    </td>
                                </tr>
                                @endforeach
