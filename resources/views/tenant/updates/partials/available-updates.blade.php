@if ($availableUpdates->isNotEmpty())
    ...
    {{-- Previous Available Updates (compact list) --}}
    @if ($olderReleases->isNotEmpty())
        <div class="overflow-hidden rounded-[28px] border border-slate-200 bg-white/92 shadow-sm dark:border-slate-800 dark:bg-slate-950/70">
            <div class="flex items-center justify-between gap-3 border-b border-slate-200 px-6 py-4 dark:border-slate-800">
                ...
                <span class="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-600 dark:bg-slate-800 dark:text-slate-300">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                @if ($release->is_prerelease)
                                    <span class="inline-flex items-center rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.16em] text-amber-700 dark:bg-amber-500/15 dark:text-amber-200">
                                        Pre-release
                                    </span>
                                @endif
                            </div>
                            <p class="mt-0.5 text-xs text-slate-600 dark:text-slate-300">{{ $release->name }}</p>
                        ...
                        <div class="shrink-0">
                            @if ($tenantCanDeployCode)
                                ...
                                >
                                    ...
                                    <button
                                        type="submit"
                                        class="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800"
                                    >
                            ...
                            @else
                                <span class="inline-flex items-center rounded-xl bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-500 dark:bg-slate-800 dark:text-slate-400">
                                    Manual Deploy
                                </span>
                            @endif
                        </div>
